...
/*following variable are added for captive portal module*/
int captive_enable = 0;
char captive_url_old[256] = {0};
...
/*
*check the status of the captive portal.
*
*/
int check_captive_portal_status(){
	...
	tr_trace_enter();
	int ret = -1;
	int captive_enable_get = 0;
	char linebuffer[512] = {0};
	char captive_url_new[256] = {0};
	char url_chang_flag = 0;
	char url_empty_flag = 0;
	char switch_flag = 0;
	char tmp[128] = {0};
	char notify_flag = 0; 
	...
	ret=do_uci_get(CAPTIVE_ENABLE,tmp);
	if(!ret){
		captive_enable_get = atoi(tmp);
		tr_dbg("get the enable option %d\n",captive_enable_get);
		
		if(captive_enable_get/* && captive_enable_get != captive_enable*/){
			/*enable = 1*/
			
			ret = do_uci_get(CAPTIVE_URL,captive_url_new);
			if(!ret){
				if(captive_url_new[0] != 0){
					if(captive_url_old[0] != 0){
						if(!strcmp(captive_url_new,captive_url_old)){
							url_chang_flag = 0;
						}else{
							url_chang_flag = 1;
						}
					...
					strcpy(captive_url_old,captive_url_new);
				}else{
					url_empty_flag = 1;
				}
			}else{/*the url is blank*/
				url_empty_flag = 1;
			}

			if(url_empty_flag){
				sprintf(linebuffer,"%s %s","Enabled","Error_URLEmpty");
			}else{
				if(url_chang_flag){
					sprintf(linebuffer,"%s %s","Enabled",captive_url_new);
				}else{
					if(captive_enable_get != captive_enable){
						sprintf(linebuffer,"%s %s","Enabled",captive_url_old);
					}
				}
			}
			
		}else if (!captive_enable_get /*&& captive_enable_get != captive_enable*/ ){
			/*enable = 0*/
			ret = do_uci_get(CAPTIVE_URL,captive_url_new);
			if(!ret){
				tr_trace_line();
				if(captive_url_new[0]!= 0){
					if(strcmp(captive_url_new,captive_url_old) !=0){
						url_chang_flag = 1;
					}else{
						url_chang_flag = 0;
					}
				...
				if(url_empty_flag){
					sprintf(linebuffer,"%s %s","Disabled","Error_URLEmpty");
				}else{
					if(url_chang_flag){
						sprintf(linebuffer,"%s %s","Disabled",captive_url_new);
					}
				}
		...
		//captive_enable = captive_enable_get;
	}

	tr_dbg("the content of the linebuffer is [%s]\n",linebuffer);

	
	captive_enable = captive_enable_get;
	if(url_empty_flag){
		tr_trace_line();
		memset(captive_url_old,0x00,sizeof(captive_url_old));
	}else{
		if(captive_url_new[0] != 0){
			tr_trace_line();
			memset(captive_url_old,0x00,sizeof(captive_url_old));
	...
	tr_dbg("do the history record!captive_enable=[%d],url=[%s]\n",captive_enable,captive_url_old);
	
	if(linebuffer[0] != 0){
		notify_flag = 1;
		tr_dbg("the captive_url_old is backuped [%s]\n",captive_url_old);
		ret = do_uci_set(CAPTIVE_STATUS,linebuffer);
		if(ret){	
			tr_err("uci set error!\n");
		}else{
			do_uci_commit("wifidog");
		}
		
	}
	if(!notify_flag){
		tr_dbg("The captive portal status is not changed!\n");
	}
