...
int managementServerPasswordFlag = 0; //ASKEY ADD
extern int runX_CharterRIP;
extern int restart_network;
extern int restart_IPv6;
extern int restart_Lan_network;
extern int restart_radvd;
extern int wifi_restart;
extern int wifi_radio_restart;
extern int portmapping_restart;
extern int dhcp_restart;
extern int dhcp_num;
extern int start_upload;
extern int start_download;
extern int set_remoteaccess;
...
void need_reboot_agent()
{
    struct session *ss;

    if( sched && sched->pdata ) {
        ss = ( struct session * )( sched->pdata );
        ss->agent_reboot = 1; /* Reboot agent when session terminated */
    }
}
...
static int http_header( struct session *ss )
{
	...
	/* ASKEY SH add start
	Support for appending a URI query string containing the following parameters to its ACS URL prior to
	every HTTP/HTTPS request made as part of a CWMP session.
	. username = Username for CWMP authentication (Device.ManagementServer.Username)
	. provcode = Value of TR-069 ProvisioningCode (Device.DeviceInfo.ProvisioningCode)
	As an example, if a device has an ACS URL of https://acs.twcable.com, a username of
	010203-TWC1000-010203040506, and a ProvisioningCode value of provisioned the URL used by the
	Device would be:
	http://acs.twcable.com?username=010203-TWC1000-010203040506&provcode=provisioned
	*/
	/* The username MUST follow the format of <OUI>-<ProductClass>-<SerialNumber> using the values of the fields from the
    DeviceIdStruct.
    */
	char ProvisioningCode[257] = {0};
	char X_CHARTER_COM_ACSIdentifier[33] = {0};
	char OuiValue[64] = {0};
	...
	GET_NODE_VALUE( "Device.DeviceInfo.ProvisioningCode", ProvisioningCode );
	GET_NODE_VALUE( "Device.ManagementServer.X_CHARTER_COM_ACSIdentifier", X_CHARTER_COM_ACSIdentifier );
	...
	if( strcasecmp( X_CHARTER_COM_ACSIdentifier, "TWC" ) == 0 )
	{
		sprintf(tmpbuff, "?username=%s-%s-%s&provcode=%s", OuiValue, ProductClassValue, SerialNumberValue, ProvisioningCode);
	}
	else
	{
		strcpy(tmpbuff, "");
    ...
    if( ss->acs || ss->cpe ) {
        ...
        if( http_chunk ) {
            push_buffer( & ( ss->outbuf ),
                         ...
                         "\r\n",
                         ss->http.req_path, tmpbuff, //ASKEY SH mod
#ifdef __DEVICE_IPV6__
                         ipv6_check ? "[" : "",
#endif /* _DEVICE_IPV6__ */
                         ss->http.req_host,
...
static void session_destroy( struct sched *sc )
{
	...
	if (ss->cpe_result == METHOD_SUCCESSED){
		if(runX_CharterRIP)
		{
			runRipFuncs();
			runX_CharterRIP = 0;
		...
		if(restart_Lan_network)
		{
			RestartLanNetwork();
			restart_Lan_network = 0;
		}
		...
		if(dhcp_restart)
		{
			Dhcprestart(dhcp_num);
			dhcp_restart = 0;
		}

		if(start_upload)
		{
			pthread_t upload_pid;
    		pthread_create( &upload_pid, NULL, ( void * ) TrUpload, NULL );
			start_upload = 0;
		}

		if(start_download)
		{
			pthread_t downloa_pid;
    		pthread_create( &downloa_pid, NULL, ( void * ) TrDownload, NULL );
			start_download = 0;
		}

		if(set_remoteaccess)
		{
			pthread_t remoteaccess_pid;
			pthread_create( &remoteaccess_pid, NULL, ( void * ) set_RemoteAccess, NULL );
			set_remoteaccess = 0;
		}
		...
		if (wifi_restart) {
			pthread_t wifi_pid;
    		pthread_create( &wifi_pid, NULL, ( void * ) runWifiReload, NULL );
			wifi_restart = 0;
		}

		if (wifi_radio_restart) {
			pthread_t wifi_radio_pid;
    		pthread_create( &wifi_radio_pid, NULL, ( void * ) runSbinWifi, NULL );
			wifi_radio_restart = 0;
		}

		if (portmapping_restart) {
			pthread_t portmapping_pid;
    		pthread_create( &portmapping_pid, NULL, ( void * ) runPortmapping, NULL );
			portmapping_restart = 0;
		}
	}
        ...
        if( factory_reset ) {
            ...
            lib_factory_reset();
			reboot = 0; //don't reboot in here, ASKEY SH add
 ...
 //			lib_reboot();
        ...
        if( agent_reboot ){
            tr_log( LOG_WARNING, "Reboot TR Agent because set parameter need reboot agent, for example ACS URL" );
            lib_agent_reboot();
        }
    }
...
int create_session( void )
{
    ...
    if( first == 1 ) {
		...
		managementServerPasswordFlag = 1; //ASKEY ADD
...
#endif
#ifdef WAN_DSL_DIAGNOSTICS
        register_vct( DSL_DIAGNOSTICS, wan_dsl_diagnostics_changed );
#endif
#ifdef WAN_ATMF5LOOPBACK_DIAGNOSTICS
        register_vct( ATM_DIAGNOSTICS, wan_ATMF5Loopback_diagnostics_changed );
#endif
        lib_end_session();
    }
    ...
    if( ant > 0 && event_only_value_change() && last_session + ant > current_time() ) {
        if( ant_sc == NULL ) {
            ...
            if( ant_sc == NULL ) {
                ...
                ant_sc->timeout = last_session + ant;
        ...
        if( session_lock == 0 && sched == NULL && cwmp_enable == BOOLEAN_TRUE && get_event_count( 0 ) > 0 && tr_exist( FLAG_BOOTSTRAP ) ) {    //Not in session at present
            ...
            if( res < 0 ) {
				...
				char tmpbuff[1024] = {0};
				do_uci_get("trconf.Device_ManagementServer.sent_transfer_complete_event", tmp);
				do_uci_get("trconf.Device_ManagementServer.sent_transfer_complete_cmd_key", cmd_key);
				do_uci_get("trconf.Device_ManagementServer.sent_transfer_complete_start_time", start_time);
				tr_log( LOG_NOTICE, "trconf.Device_ManagementServer.sent_transfer_complete_event: %d", atoi(tmp));
				tr_log( LOG_NOTICE, "trconf.Device_ManagementServer.sent_transfer_complete_cmd_key: %s", cmd_key);
				tr_log( LOG_NOTICE, "trconf.Device_ManagementServer.sent_transfer_complete_start_time: %s", start_time);
				if(atoi(tmp) == 1)
				{
					FILE *fp = NULL;
					fp = fopen("/tmp/beforreboot_transfer", "r");				
					if(fp != NULL)
					{
						tr_log( LOG_NOTICE, "beforreboot_transfer!");
						fclose(fp);
					}
					else
					{
						sprintf(tmpbuff, "<CommandKey>%s</CommandKey>\n<FaultStruct>\n<FaultCode>0</FaultCode>\n<FaultString>%s</FaultString>\n</FaultStruct>\n<StartTime>%s</StartTime>\n<CompleteTime>%s</CompleteTime>\n", 
											cmd_key, "", start_time, lib_current_time());
						//add_single_event( M_EVENT_DOWNLOAD );
						//add_single_event( S_EVENT_TRANSFER_COMPLETE );
						add_request( "TransferComplete", -1, cmd_key, tmpbuff );
						do_uci_set("trconf.Device_ManagementServer.sent_transfer_complete_event", "0");
						do_uci_commit("trconf");
						tr_log( LOG_NOTICE, "url: %s", url);
					}
				}
				//Add end
                res = 0;
