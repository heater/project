...
int node_is_instance_askey( node_t node ); //ASKEY ADD
 ...
 */
int node_is_instance_askey( node_t node )
{
    ...
    if( path_len <= 0 || path[path_len - 1] != '.' || path[0] == '.' ) {
        ...
        if( res < 0 ) {
            ...
            if( node_is_instance( node ) ) {
                ...
                return 9004; /* mod [Vincent: 2016/09/29: del object fail return error code from 9005 to 9004]*/
            }
        }
    }
