...
#ifdef CLI_USE_UDS
int tr_listen_unix_domain_socket( const char *sock_name, int type, int backlog )
{
    int fd;

    fd = war_socket( PF_UNIX, type, 0 );
    ...
    if( fd >= 0 ) {
        ...
        close_on_exec( fd );
        reuse_socket_addr( fd );
        if( bind( fd, ( struct sockaddr * ) &listen_addr, strlen(sock_name) + sizeof(listen_addr.sun_family) ) < 0 ) {
            tr_log( LOG_ERROR, "Bind failed: %s\n", war_strerror( war_geterror() ) );
            war_sockclose( fd );
            fd = -1;
        } else {
