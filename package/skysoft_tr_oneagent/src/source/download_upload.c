...
int download_upload_process( struct session *ss, char **msg )
{
    ...
    while( ss->cpe_pdata == ( void * )0 && xml_next_tag( msg, &tag ) == XML_OK ) {
        if( strcasecmp( tag.name, "CommandKey" ) == 0 ) {
            ...
            if( tag.value && *( tag.value ) ) {
                ...
                if( strchr( tag.value, '[' ) != NULL && strchr( tag.value, ']' ) != NULL )
                    t->ipv6_sb = 1;
	...
	if(t->type == FT_TYPE_UPLOAD)
	{
		...
		char mac1[32] = {0};
		char tmpurl[257] = {0};
		char tmpfile_type[65] = {0};
		char *ptr = NULL;
		
		getInfaceWanMac(mac); //Using WAN MAC address
		tr_log(LOG_DEBUG,"WAN Iinterface MAC [%s]",mac);
		tr_log(LOG_DEBUG,"t->url[%s]",t->url);
		tr_log(LOG_DEBUG,"strlen t->url[%d]",strlen(t->url));
		tr_log(LOG_DEBUG,"t->url[%s]",t->file_type);
		if (mac[0] != '\0'){
			int i = 0, j = 0;
			for (i = 0; i < 17; i ++){
				if (mac[i] != ':'){
		...
		strcpy(tmpfile_type, t->file_type);
		ptr = tmpfile_type;
		while(*ptr != '\0')
		{
			if(*ptr == ' ')
		...
		tr_log(LOG_DEBUG,"tmpfile_type[%s]",tmpfile_type);
		if(t->url[strlen(t->url)-1] != '/')
		{
			ptr = strrchr(t->url, '/');
			if(ptr != NULL)
			{
				if(*(ptr-1) == '/')
				{
					sprintf(tmpurl, "%s/%s_%s", t->url, mac1, tmpfile_type);
				}
				else
				{
					*ptr = '\0';
					sprintf(tmpurl, "%s/%s_%s", t->url, mac1, ptr+1);
				}
			}
		}
		else
    ...
    if( ss->cpe_pdata == ( void * )0 ) {
        t->need_queue = 1;
		//ASKEY MOD/s, don't download FW after reboot
        //t->need_persist = 1;
        t->need_persist = 0;
		//ASKEY MOD/e
