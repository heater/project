...
int gpv_gpa_process( struct session *ss, char **msg )
{
    ...
    while( xml_next_tag( msg, &tag ) == XML_OK ) {
        if( war_strcasecmp( tag.name, "ParameterNames" ) == 0 ) {
			...
			/* wangzheng for init dynamic object instance 2016-01-28*/
			char path_temp[NODE_PATH_LEN + 1] = "";
			snprintf(path_temp, sizeof( path_temp ), "%s", path);
			if (end_with_dot)
			{
				if (path_temp[0] == '\0')
				{
					lib_dynamic_init_children_frmgetv("Device.");
				}
				else
				{
					lib_dynamic_init_children_frmgetv(path_temp);
				}
			}
			/* end */
