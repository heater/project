...
#endif
#ifdef CLI_USE_UDS
static char sock_name[128] = "/var/oneagent_cli.sock";
#endif
...
static void cli_value_change_group( struct cli *cli, char *not_used )
{
    char *name = NULL, *temp = NULL, **record = NULL;
    int i, count = 0;
    ...
    if( name == NULL || count == 0 ) {
		...
		//get parameter name pointers
		temp = name;
		record = malloc( count*sizeof(char*) );
		for( i=0; i<count; i++ )
		{
			record[i] = temp;
			temp = strchr( temp, ';' );
		...
		for( i=0; i<count; i++ )
		{
			node_t node;
			char *value;
			//tr_log( LOG_NOTICE, "xxxxxxxx record[%d] ==%s",i, record[i]);
			//tr_log( LOG_ERROR,  "xxxxxxxx node ==%s", node);
			if( lib_resolve_node( record[i], &node ) == 0 )
			{
				if( !lib_get_value(node, &value) ){
					//tr_log( LOG_ERROR, "xxxxxxxx value==%s",value );
					__add_inform_parameter( record[i], 1, value );
...
{
    int client;
#if defined(CLI_USE_UDS)
    struct sockaddr_un cli_addr;
#elif defined(__DEVICE_IPV4__)
...
#ifdef CLI_USE_UDS
    cli = tr_listen_unix_domain_socket( sock_name, SOCK_STREAM, 1 );
#else
