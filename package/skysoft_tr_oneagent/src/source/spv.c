...
struct datebase_type g_datebase_type[] = {{"Alias","string"},{"Dbm1000","int"},{"IEEE_EUI64","string"},{"IPAddress","string"},{"IPPrefix","string"},
                                                                   {"IPv4Address","string"},{"IPv4Prefix","string"},{"IPv6Address","string"},{"IPv6Prefix","string"},{"ZigBeeNetworkAddress","string"},
                                                                   {"MACAddress","string"},{"StatsCounter32","unsignedInt"},{"StatsCounter64","unsignedLong"},{"UUID","string"}};
...
int set_status_1( const char *name, const char *path )
{
    ...
    if( status_1_head == NULL ) {
		...
		status_1_prev = status_1_head;
		while (status_1_prev->next)
		{
			status_1_prev = status_1_prev->next;
		}
...
int del_status_1( struct status_1 * cur_status_1 )
{
    if (status_1_head == cur_status_1)
    {
          free(cur_status_1);
	      status_1_head = NULL;
    }
    else
    {
	   ...
	   while (status_1_prev->next)
	   {
	        if (status_1_prev->next == cur_status_1)
	        {
	            status_1_prev->next = cur_status_1->next;
		        free(cur_status_1);
		        break;
	        }
...
int __backup_set_parameter_value( const char *_path, const char *value )
{
    ...
    if( spv_journal( path, old_value ) || ( lib_set_value( node, value ) != 0 ) ) {
...
int __set_parameter_value( const char *_path, const char *value )
{
    ...
    if(strcmp(path,path_pt))/*skysoft*/
...
extern int parameternum; //Askey add
...
/* tr106 a7 */
	     ...
	     memset(basetype,0,32);
	     GET_ARRAY_LEN(g_datebase_type,typelen)
		 	
	     for(typei=0; typei<typelen; typei++) {
	       if (war_strcasecmp(g_datebase_type[typei].datetype, type) == 0) {
		    strcpy(basetype,g_datebase_type[typei].basetype);
		    break;
	        }
	     }

 /*        if( war_strcasecmp( type, ptype ) != 0 ) { */
           if(( war_strcasecmp( type, ptype ) != 0 )  && ( war_strcasecmp( basetype, ptype ) != 0 ) ){
                s->fault_code = 9003;
                node->fault_code = 9006;
                goto complete;
            }
        ...
        if( s->fault_code == 0 ) {
            ...
            if( lib_get_value( node->node, &v ) != 0 ) {
                ...
                goto internal_error;
            } else if( ( strcmp( v ? v : "", val ) == 0 ) && ( strcmp(node->node->name,"ConnectionRequestPassword") != 0 ) && (parameternum != 0) && ( strcmp(node->node->name,"Alias") != 0 )) { /* No change from old version */
                ...
                goto complete;
            } else if ( spv_journal( path, v ) != 0 || (res = lib_set_value( node->node, val )) != 0 ) {
                if( s->crypt ) {
                    if( val ) {
                        free( val );
                    }
				...
				//ASKEY SH add start
				else if (res == -3) //for send error code 9001(Request denied)
				{
					s->fault_code = 9001;
                    //node->fault_code = 9007;
					goto complete;
...
int spv_process( struct session *ss, char **msg )
{
    ...
    while( xml_next_tag( msg, &tag ) == XML_OK ) {
        if( war_strcasecmp( tag.name, "ParameterList" ) == 0 ) {
			...
			parameternum = exp_num; //Askey add
        } else if( war_strcasecmp( tag.name, "ParameterValueStruct" ) == 0 ) {
            act_num++;
			parameternum--; //Askey add
            if( spv( msg, s ) != 0 ) {
                break;
            }
