...
static char serialnu2[65] = ""; //ASKEY SH add
extern a_infomparaInfo ip_map[]; //ASKEY SH add
...
#define EXIST_EVENT(code) ((events & (1 << code)) != 0)
    ...
    /* boot should not always follow bootstrap, askey judge boot with /var/.FLAG_BOOT flag, so remove this
...
void check_para()
{
	...
	while( next_inform_parameter( &i ) ) {
        ...
        if(i->commited == 1)
        	{
        		for (cc = 0; cc < INFORMPARA_MAX_INSTANCE_NUM; cc ++)
				{
					//tr_log(LOG_DEBUG,"ParameterName[%s],i->path[%s],ip_map[cc].Enable[%d]", ip_map[cc].ParameterName,i->path,ip_map[cc].Enable);
					if (strcmp(ip_map[cc].ParameterName,i->path) == 0)	
					{
						sprintf(name, "%s%d", DMIt_Enable, cc+1);
						ret = do_uci_get(name, value);
						if (ret)
						{ 
							if (ip_map[cc].Enable == 0)
							{
								flag = 1;
							}
						}
				...
				//if (cc == INFORMPARA_MAX_INSTANCE_NUM || flag == 1)
				if (flag == 1)
				{
					tr_log(LOG_DEBUG,"set commited to 0");
					i->commited = 0;
				}
        	}
    }
...
int inform_process( struct session *ss )
{
    ...
    if( first == 1 ) {
		...
		GET_NODE_VALUE( DEVICE_ID_SERIAL_NUMBER2, serialnu2 ); //ASKEY SH add
    ...
    switch( next_step ) {
        case INFORM_DEVICE_ID: {
                ...
                push_soap( ss,
                           ...
                           "<X_CHARTER_COM_SerialNumber>%s</X_CHARTER_COM_SerialNumber>" //ASKEY SH add
                           "</DeviceId>\n", xml_man ? xml_man : man, oui, xml_pc ? xml_pc : pclass, xml_sn ? xml_sn : serialnu, xml_sn2 ? xml_sn2 : serialnu2 ); //ASKEY SH mod
        ...
        case INFORM_EVENT: {
                ...
                while( next_event( &e ) ) {
                    if( e->commited != 0 ) {
						...
						if(e->event_code == S_EVENT_TRANSFER_COMPLETE)
						{
							tr_log( LOG_NOTICE, "echo 0 > /tmp/waitacs *** event_code: %s", code2string( e->event_code ));
							system("echo 0 > /tmp/waitacs");
						}
        ...
        case INFORM_PARAMETER: {
                ...
                while( next_inform_parameter( &next_ip ) ) {
					//tr_log(LOG_DEBUG,"INFORM_PARAMETER_HEADER next_ip->path[%s], next_ip->commited[%d]",next_ip->path, next_ip->commited);
