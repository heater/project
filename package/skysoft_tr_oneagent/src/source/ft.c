    ...
    if( ( t->tw[0].max_retries == -1 || t->tw[0].max_retries >= 1 ) && !FINAL_FAULT_CODE( t->fault_code ) ) {
        ...
        switch( t->type ) {
            ...
            case FT_TYPE_SCHEDULE_DOWNLOAD:
                if( t->fault_code == 0 ) {
                    http_destroy( &( t->http ) );
					do_uci_set("trconf.Device_ManagementServer.sent_transfer_complete_start_time", t->start_time); //ASKEY SH add
					
                    res = lib_download_complete( t->file_type, t->file_path, t->cmd_key );

                    if( res > 0 ) {
                        //printf("=============James debug====Firmware Path:%s\n", t->file_path);
                        if (strstr(t->file_path, "image") != NULL){ //ASKEY, don't reboot in here for FW upgrading
                            //add_single_event( M_EVENT_DOWNLOAD ); //to send event
...
static void task_add_tc_request( struct task *t, int need_reboot, const char *fault_string )
{
    ...
    if( get_event( TASK_IS_DOWNLOAD( t ) ? ( t->type == FT_TYPE_DOWNLOAD ? M_EVENT_DOWNLOAD : M_EVENT_SCHEDULE_DOWNLOAD ) :
		...
		if(atoi(tmp) != 1)
		{
			tr_log( LOG_NOTICE, "+++++++++++++++++++++++++++ add_single_event: %d", S_EVENT_TRANSFER_COMPLETE);
