...
extern int lib_ao_commit(node_t node, uint32_t instance); //ASKEY ADD
...
int __ao__( char *path, int path_len )
{
    ...
    if( path_len <= 0 || path[path_len - 1] != '.' || path[0] == '.' ) {
        ...
        if( res < 0 ) {
            return 9002;
/* mod [Vincent: 2016/09/29: add object fail return error code from 9005 to 9004]*/
        } else if( res > 0  ) {
            return 9005;
		} else if( multi_instance_capable( node ) != 1 ) {
            return 9004;
/* mod/e */
