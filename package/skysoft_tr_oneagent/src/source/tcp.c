...
short tcplistenport = 7547;

int connectionRequestPasswordFlag = 0; //ASKEY ADD
...
{
        tr_log( LOG_DEBUG, "enable_changed" );
	if( strcmp(new,"0") == 0) {
		node_t node1,node2;
    		lib_start_session();
    		lib_resolve_node(CRS_URL , &node1 );
    		lib_set_value( node1, " ");
   		lib_end_session();
    		if( sched ) {
...
static void url_changed( const char *path1, const char *new ) //ASKEY SH mod
{
    char *h, *p, *pt;
    uint16_t port_tmp;
    ...
    if( new == NULL || *new == '\0' ) {
		...
		tr_log(LOG_DEBUG,"url_changed, URL=%s",url);
        ...
        if( *h == '[' ) {
			...
			strcpy(addr, ipv6addr);
			*/
			strcpy(addr, "::"); //for IPv6
			// ASKEY SH add/e
            ...
            *p = '\0';
            ...
            if( p == NULL ) {
				/*port_tmp = 7547; *//*Askey Wangzheng 2015-07-29: not same with udp port */
				port_tmp = tcplistenport;
            } else {
                *p = '\0';
                port_tmp = atoi( p + 1 );
            }

			strcpy(addr, "0.0.0.0"); //for IPv4, ASKEY SH add
        }
...
#define TCP_GET_CONFIG(path, buf) do { \
		...
		connectionRequestPasswordFlag = 1; //ASKEY ADD
        ...
        register_vct( CRS_ENABLE, enable_changed ); //ASKEY SH add
        register_vct( CRS_URL, url_changed ); //ASKEY SH add
        first = 0;
        url_changed("", url ); //ASKEY SH mod
        return 0;
    }
	...
	tr_log(LOG_DEBUG,"launch_tcp_listener, listen addr=%s, port=%d", addr, port);
