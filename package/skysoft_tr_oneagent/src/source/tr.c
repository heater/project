...
void tr_reset_safe_exit_timer(struct sched * sesc)
{
    time_t curr_time = 0;
    
    curr_time = current_time();
    sesc->timeout = curr_time + 1;
}

void tr_periodic_check_exit(struct sched * sesc)
{
    int session_count = 0;

    session_count = lib_get_session_count();
    if(tr_exist( FLAG_TREE_TO_XML )){
        tr_log(LOG_DEBUG, "TRAgent is busy in writing tree to xml, exit later!");
        tr_reset_safe_exit_timer(sesc);
    }
    else if(session_count != 0){
        tr_log(LOG_DEBUG, "TRAgent is busy, session count is %d ,exit later!", session_count);
        tr_reset_safe_exit_timer(sesc);
    }
    else{
        tr_log(LOG_DEBUG, "TRAgent is idle, exit right now!");
        _exit(1);
    }
}
...
void tr_safe_exit()
{
    struct sched *sesc = NULL;

    sesc = calloc( 1, sizeof( *sesc ) );
    sesc->type = SCHED_WAITING_TIMEOUT;
    sesc->fd = -1;
    tr_reset_safe_exit_timer(sesc);
    sesc->on_timeout = tr_periodic_check_exit;
    sesc->on_destroy = NULL;
#ifdef CODE_DEBUG
    sesc->name = "Safe Exit";
#endif
    add_sched( sesc );
    tr_log( LOG_DEBUG, "Launch safe exit sched" );
}

void tr_signal_handler(int signo)
{
    if (signo == SIGTERM){
        tr_log( LOG_DEBUG, "Received SIGTERM signal" );
        tr_safe_exit();
    }
}
...
void tr_init_signal()
{
    ...
    act.sa_handler = tr_signal_handler;
    act.sa_flags = 0;
...
#endif
{
    ...
    if( lib_init_mapping_files() != 0 )
    {
        tr_log( LOG_ERROR, "Init mapping files failed!" );
    }
    else
    {
        tr_log( LOG_ERROR, "Init mapping files OK !" );
    }
...
#ifdef TR069_WIB
        launch_wib_sched();
#else /*If the FLAG_BOOTSTRAP exists, it means it is not the FIRST INSTALL */
        tr_create( FLAG_BOOTSTRAP );
#endif
    }
    
    /* If the FLAG_BOOT exists, it means it is not BOOT (Power On), but agent restart */
    if( !tr_exist( FLAG_BOOT ) ) {
    ...
    if( tr_exist( FLAG_BOOTSTRAP ) ) {
        ...
        complete_add_event( 0 );
#ifdef TR111
//ASKEY SH, fix STUN doesn't work issue with open ipv6
